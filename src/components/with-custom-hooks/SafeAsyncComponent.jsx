import {useSafeAsync} from 'leakguard-safe-hooks/src/hooks/useSafeAsync';
...
export default function SafeAsyncCompoent() {
    const {status, result: user, error} = useSafeAsync(loadUser, [], {
        taskName: "LoadUser",
        keepAlive: true
    });
    ...
    return (
        <div >
            ...
            {
                error && (
                    <p className="text-red-600 font-semibold">❌ 오류 발생: {error.message}</p>
                )
            }
